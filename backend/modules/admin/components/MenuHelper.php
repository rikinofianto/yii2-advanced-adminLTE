...
class MenuHelper
{
    ...
    public static function setActiveMenu(&$menus, $route)
    {
        foreach ($menus as &$item) {
            // Cek apakah URL cocok dengan rute saat ini
            if (strpos($route, preg_replace('/\/[^\/]+$/', '', trim($item['url'][0], '/'))) !== false) {
                $item['active'] = true;
            } else {
                $item['active'] = false;
            }
            
            // Jika ada sub-items, lakukan pengecekan rekursif
            if (isset($item['items']) && is_array($item['items'])) {
                self::setActiveMenu($item['items'], $route);
                
                // Jika salah satu sub-item aktif, set parent item sebagai active
